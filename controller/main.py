"""minio-provisioner operator entry point.
...
import buckets  # noqa: F401 — registers MinioBucket handlers
import kopf
import users  # noqa: F401 — registers MinioUser handlers


@kopf.on.startup()
def configure(settings: kopf.OperatorSettings, **_: Any) -> None:
    settings.peering.standalone = True
    settings.persistence.finalizer = "minio.sonne.zone/finalizer"
    ...
    # Explicit thread-pool size for sync handlers (minio SDK calls).
...
def on_shutdown(logger: kopf.Logger, **_: Any) -> None:
    logger.info("minio-provisioner shutting down gracefully")
